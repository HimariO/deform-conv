...
from dataset_gen import NPZ_gen, NPZ_class_id
...
n_train = (88000 + 16000) * 1  # Currenly using 200*200 & mtcnn 65*65 dataset
# n_train = 300
...
with tf.Session(config=config) as sess:
    ...
    # ---
    # Deformable CNN
    model = get_large_deform_cnn2(class_num, trainable=True, GPU=GPU_NUM)
    # model = Model(inputs=inputs, outputs=outputs)

    model.summary()
    print('[train_debug] ', model.inputs)
    print('[train_debug] ', model.outputs)
    ...
    model.compile(optim, loss=[loss, lambda y_true, y_pred: K.sum(y_pred)], loss_weights=[1., .2], metrics={'output': 'accuracy'})
    checkpoint = ModelCheckpoint("deform_cnn_best.h5", monitor='val_acc', save_best_only=True)
    checkpoint_tl = ModelCheckpoint("deform_cnn_trainbest.h5", monitor='loss', save_best_only=True)
    ...
    if args.img_dir is None:
        try:
            ...
            model.save_weights('deform_cnn_interrupt.h5')
    else:
        if True:
